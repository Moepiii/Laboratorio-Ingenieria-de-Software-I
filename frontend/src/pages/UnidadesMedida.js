...
import { useParams } from 'react-router-dom'; // ⭐️ Importar useParams
...
const UnidadesMedida = () => {
    ...
    }, [token, proyectoId, currentUser.username]);

    useEffect(() => { 
        if (proyectoId) {
            fetchUnidades(); 
        }
    }, [fetchUnidades, proyectoId]);
    ...
    const handleSubmit = async (e) => {
        ...
        const dataToSend = {
            ...
            proyecto_id: proyectoId // ⭐️ AÑADIMOS EL PROYECTO ID AL CREAR
        };
    ...
    const handleDelete = async (unidadId) => {
        if (window.confirm("¿Estás seguro de eliminar esta unidad?")) {
            try {
                await deleteUnidad(token, unidadId, currentUser.username);
                fetchUnidades();
            } catch (error) {
                alert("Error: " + error.message);
            }
                ...
                <table style={styles.table}>
                    ...
                    <tbody>
                        {unidades.length > 0 ? unidades.map((u) => (
                            ...
                            <tr><td colSpan="6" style={{...styles.td, textAlign:'center'}}>No hay unidades para este proyecto.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={currentUnidad ? "Editar Unidad" : "Añadir Unidad"}>
                <form onSubmit={handleSubmit} style={styles.form}>
                    <div style={styles.formGroup}>
                        ...
                        <input type="text" id="nombre" name="nombre" value={formData.nombre} onChange={handleChange} required style={styles.input} placeholder="Ej. Kilogramo" />
                    </div>
                    <div style={styles.formGroup}>
                        <label htmlFor="abreviatura" style={styles.label}>Abreviatura</label>
                        <input type="text" id="abreviatura" name="abreviatura" value={formData.abreviatura} onChange={handleChange} required style={styles.input} placeholder="Ej. kg" />
                    </div>
                    <div style={styles.formGroup}>
                        <label htmlFor="dimension" style={styles.label}>Dimensión (Número Real)</label>
                        <input type="number" step="any" id="dimension" name="dimension" value={formData.dimension} onChange={handleChange} required style={styles.input} placeholder="Ej. 10.5" />
                    </div>
                    <div style={styles.formGroup}>
                        <label htmlFor="tipo" style={styles.label}>Tipo</label>
                        <select id="tipo" name="tipo" value={formData.tipo} onChange={handleChange} style={styles.select}>
