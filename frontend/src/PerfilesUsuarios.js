...
const styles = {
  ...
  // NUEVOS ESTILOS PARA EL DROPDOWN DE ASIGNAR
  selectAssign: {
    padding: '0.5rem',
    borderRadius: '6px',
    border: '1px solid #d1d5db',
  ...
  // (Añadimos más estilos que puedan faltar)
  label: {
    display: 'block',
    fontSize: '0.875rem',
    fontWeight: '500',
...
const PerfilesUsuarios = ({ apiCall, currentUser }) => {
  ...
  // --- NUEVOS ESTADOS PARA PROYECTOS ---
  const [proyectosList, setProyectosList] = useState([]); // Lista de proyectos para el dropdown
  const [selectedProyectos, setSelectedProyectos] = useState({}); // { [userId]: proyectoId }


  // Función para obtener la lista de usuarios (ACTUALIZADA)
  const fetchUsersAndProyectos = useCallback(async () => {
    ...
    try {
      ...
      if (usersResult.success) {
        ...
        // Inicializar el estado de los dropdowns con los valores actuales
        const initialSelections = {};
        fetchedUsers.forEach(user => {
          // user.proyecto_id puede ser null, 0, o un ID.
          // El valor '0' lo usaremos para "No asignado" en el select.
      ...
      // 2. Obtener Proyectos (para los dropdowns)
  ...
  // --- FUNCIÓN: Añadir Usuario (Sin cambios) ---
  const handleAdminAddUser = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setNewNombre('');
        setNewApellido('');
        fetchUsersAndProyectos(); // Volver a cargar todo
      } else {
  ...
  // --- FUNCIÓN: Borrar Usuario (Sin cambios) ---
  ...
  // --- FUNCIÓN: Cambiar Rol (Sin cambios) ---
  ...
  const handleAssignProyecto = async (userId, proyectoId) => {
    ...
    // El ID del proyecto será 0 si seleccionan "No asignado"
    ...
    try {
      ...
      if (result.success) {
        setAdminSuccess(result.data.mensaje);
        fetchUsersAndProyectos(); // Recargamos todo para ver el cambio
      } else {
  ...
  // Función para manejar el cambio en el dropdown de un usuario
  ...
  return (
    <div>
      ...
      {/* --- Formulario para Añadir Usuario (Sin cambios) --- */}
      <div style={styles.adminFormContainer}>
        ...
        <form onSubmit={handleAdminAddUser} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <button type="submit" style={{ ...styles.button, width: '100%', backgroundColor: '#10b981' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = '#059669'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = '#10b981'} disabled={loadingAdd}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                ...
                <th style={{ padding: '0.75rem', fontSize: '0.875rem', color: '#6b7280' }}>Proyecto Actual</th>{/* NUEVO */}
                <th style={{ padding: '0.75rem', fontSize: '0.875rem', color: '#6b7280' }}>Asignar Proyecto</th>{/* NUEVO */}
            ...
            <tbody>
                ...
                return (
                  <tr key={user.id} style={{ borderBottom: index < users.length - 1 ? '1px solid #f3f4f6' : 'none', backgroundColor: isSelf ? '#fffbeb' : (user.role === 'admin' ? '#f5f3ff' : 'white') }}>
                    ...
                    <td style={{ padding: '0.75rem' }}>
                      <span style={{ padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: '700', backgroundColor: user.role === 'admin' ? '#eef2ff' : '#d1fae5', color: user.role === 'admin' ? '#4f46e5' : '#065f46' }}>
                    ...
                    {/* NUEVO: Asignar Proyecto (Dropdown + Botón) */}
                    <td style={{ padding: '0.75rem' }}>
                      {isSelf ? (
                        <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>N/A (Tú)</span>
                      ) : (
                    ...
                    {/* Acciones (Cambiar Rol, Borrar) */}
                    <td style={{ padding: '0.75rem', textAlign: 'center', minWidth: '220px' }}>
                      <button
                        onClick={() => handleAdminUpdateRole(user.id, user.username, user.role)}
                        style={{ padding: '0.5rem 1rem', borderRadius: '6px', fontSize: '0.875rem', fontWeight: '600', marginRight: '0.5rem', backgroundColor: user.role === 'admin' ? '#f87171' : '#34d399', color: 'white', border: 'none', cursor: isSelf ? 'not-allowed' : 'pointer', opacity: isSelf ? 0.5 : 1, transition: 'background-color 0.2s' }}
                      ...
                      <button
                        ...
                        style={{ padding: '0.5rem 1rem', borderRadius: '6px', fontSize: '0.875rem', fontWeight: '600', backgroundColor: '#ef4444', color: 'white', border: 'none', cursor: isSelf ? 'not-allowed' : 'pointer', opacity: isSelf ? 0.5 : 1, transition: 'background-color 0.2s' }}
              ...
              {users.length === 0 && !loadingUsers && (
                <tr><td colSpan="8" style={{ textAlign: 'center', padding: '1rem', color: '#6b7280' }}>No hay usuarios para mostrar.</td></tr>
              )}
            </tbody>
          </table>
        </div>
